...
function ErrorDialog({
  ...
  return (
    ...
    >
      <DialogContent className="bg-red-700 text-white max-w-4xl" aria-describedby={undefined}>
        <DialogHeader>
...
export function InstrumentationProvider({
  ...
  useEffect(() => {
    const handleError = async (event: ErrorEvent | Event) => {
      try {
        if (!('message' in event) || !event.message || event.message === '{"isTrusted":true}' || event.message === "[object Event]" || event.message === "Script error.") {
    ...
    const handleRejection = async (event: PromiseRejectionEvent) => {
      try {
        ...
        if (typeof event.reason === 'object' && event.reason !== null) {
           if ('isTrusted' in event.reason && !event.reason.message) shouldIgnore = true;
           try {
              if (JSON.stringify(event.reason) === '{"isTrusted":true}') shouldIgnore = true;
           } catch(e) {}
        }
        if (typeof event.reason === 'string' && event.reason === '{"isTrusted":true}') shouldIgnore = true;

        const errorMessage = typeof event.reason === 'string' ? event.reason : (event.reason.message || JSON.stringify(event.reason));
        
        if (errorMessage.includes("WebSocket closed without opened") || errorMessage.includes("failed to connect to websocket")) {
           shouldIgnore = true;
        }
